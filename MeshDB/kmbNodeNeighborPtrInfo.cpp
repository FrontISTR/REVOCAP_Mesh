...
{
	if( element ){
		// Ç±ÇÃóvëfÇ™ê∂ê¨Ç∑ÇÈó◊ê⁄ä÷åWÇìoò^Ç∑ÇÈ
...
{
	if( element ){
		// Ç±ÇÃóvëfÇ™ê∂ê¨Ç∑ÇÈó◊ê⁄ä÷åWÇçÌèúÇ∑ÇÈ
		const unsigned int len = element->getNodeCount();
		for(unsigned int i=0;i<len;++i){
			kmb::nodeIdType nodeId = element->getCellId(i);
...
kmb::NodeNeighborPtrInfo::getElementNeighbor( const kmb::Element* element, kmb::Element** neighbors ) const
{
	...
	// coboundaries Ç©ÇÁåvéZÇ∑ÇÈ
	// é©ï™é©êgÇÃí∏ì_îzóÒÇ©ÇÁ
	// é¸ï”óvëfÇéÊÇËèoÇµÇƒÅA
	// ÇªÇÍÇ∆ÇÃóvëfä‘ä÷åWÇåvéZÇ∑ÇÈ

	const int boundaryNum = element->getBoundaryCount();

	// Segment Segment2 ÇÃéûÇÕì¡ï 
	switch( element->getDimension() )
	{
		case 1:
		{
			for(int i=0;i<2;++i)
			{
				...
				// í∏ì_Ç≤Ç∆ÇÃé¸ï”óvëfÇ∆ÇÃä÷åWÇí≤Ç◊ÇÈ
				...
				{
					// àŸÇ»ÇÈóvëfÇ™êﬂì_Ç∆ã§óLÇµÇƒÇ¢ÇΩÇÁìoò^Ç∑ÇÈ
		...
		case 2:
		{
			...
			for(int i=0;i<boundaryNum;++i){
				...
				while( eIter != eIterPair.second && neighbors[i] == NULL )
				{
					// àŸÇ»ÇÈóvëfÇ™ê⁄ÇµÇƒÇ¢ÇΩÇÁìoò^Ç∑ÇÈ
