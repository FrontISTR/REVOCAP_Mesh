		...
		switch( data->getBindingMode() ){
				...
				while( !dIter.isFinished() ){
					...
					if( body != NULL ){
						...
						while( eIter != body->end() )
						{
							...
							for(int i=0;i<len;++i){
								nodeSet.insert( eIter.getNodeId(i) );
							}
							++eIter;
						}
					}
			...
			case kmb::DataBindings::ElementGroup:
			{
				...
				while( !dIter.isFinished() ){
					...
					for(int i=0;i<len;++i){
						nodeSet.insert( element.getNodeId(i) );
					}
					++dIter;
				}
				break;
			...
			case kmb::DataBindings::ElementGroup:
				{
					...
					while( !eIter.isFinished() ){
						...
						if( !elem.isFinished() ){
							...
							for(int i=0;i<len;++i)
							{
								if( this->getNode( elem.getNodeId(i), node ) ){
									bbox.update( node );
								}
							}
						}
					...
					while( !fIter.isFinished() ){
						if( fIter.getFace( f ) ){
							...
							if( !elem.isFinished() ){
								...
								for(int i=0;i<len;++i)
								{
									if( this->getNode( elem.getBoundaryNodeId(f.getLocalFaceId(),i), node ) ){
										bbox.update( node );
									}
								}
							}
			...
			while( !eIter.isFinished() ){
				...
				for(int i=0;i<boundaryCount;++i){
					...
					for(int j=0;j<boundaryNodeCount;++j){
						if( !nodeGroup->hasId( elem.getBoundaryNodeId(i,j) ) ){
							isFace = false;
							break;
						}
					}
		...
		if( elements ){
			...
			while( !eIter.isFinished() ){
				...
				for(int i=0;i<nodeCount;++i){
					if( !nodeGroup->hasId( elem.getNodeId(i) ) ){
						isElement = false;
						break;
					}
				}
		...
		if( elements ){
			...
			while( !fIter.isFinished() ){
				...
				if( !elem.isFinished() ){
					...
					for(int i=0;i<len;++i){
						if( nodeGroup->addId( elem.getBoundaryNodeId( localIndex, i ) ) ){
							++dataCount;
						}
					}
				}
		...
		if( elements ){
			...
			while( !eIter.isFinished() ){
				...
				if( !elem.isFinished() ){
					...
					for(int i=0;i<len;++i){
						if( nodeGroup->addId( elem.getNodeId( i ) ) ){
							++dataCount;
						}
					}
				}
